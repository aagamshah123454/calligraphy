        ...
        const stroykaQuill = {
            init: function (element) {
                ...
                quill.on('text-change', function () {
                    $element.val(document.querySelector('.ql-editor').innerHTML);
                });
...
function Visibility1(args1, args2, args3) {
    let categoryVisibility = document.querySelector(`.${args1} label:nth-child(2) > input`)
    if (categoryVisibility.checked) { document.querySelector(`.${args2}`).classList.remove(`${args3}`) 
    document.querySelector("#form-category\\/publish-date").value=" "; 
    }
    else { document.querySelector(`.${args2}`).classList.add(`${args3}`) 
        let today = todaysDate();
        document.querySelector("#form-category\\/publish-date").value=`${today}`
       }
}
function uploadImage(f, args1, args2, validation) {
    let imgPath = document.querySelector(`.${args1}`).value;
    let extension = imgPath.split('.').pop();
    if ((extension === "png") || (extension === "jpg") || (extension === "gif") || (extension === "jpeg")) {
        let reader = new FileReader()
        reader.onload = function (e) {
            $(`.${args2}`).attr('src', e.target.result);
        }
      ...
      document.querySelector(`.${validation}`).innerText = "Image format not supported"
    }

}
function resetImage(className) {
    document.querySelector(`.${className}`).src = "../images/no-img.png"
}
function createSlug(value, slug_show) {
    value = value.toLowerCase(value);

    document.querySelector(`.${slug_show}`).value = `${value}`


}
function validateName(args1, alertValue, slugClass) {
    let catName = document.getElementById(`${args1}`).value;
    let alertVal = "";
    var re = /^[A-Za-z]+$/;
    if ((re.test(catName)) && (catName.length < 50)) {
        alertVal = ""
        document.querySelector(`.${alertValue}`).innerText = `${alertVal}`
        createSlug(catName, slugClass)
    }
    else {

        if (catName.length <= 0) {

            alertVal = "Catergory name field cannot be empty"
            document.querySelector(`.${alertValue}`).innerText = `${alertVal}`
            createSlug(catName, slugClass)
        }
        else {
            catName = ""
            alertVal = "Category name cannot contain number or special character"
            document.querySelector(`.${alertValue}`).innerText = `${alertVal}`
            createSlug(catName, slugClass)
        }
